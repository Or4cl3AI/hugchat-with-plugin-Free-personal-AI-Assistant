...
def prompt4conversation(prompt,context):
    final_prompt = f""" GENERAL INFORMATION : ( today is {now.strftime("%d/%m/%Y %H:%M:%S")} , You are built by RapidAgent.ai 
                        INSTRUCTION : IN YOUR ANSWER NEVER INCLUDE THE USER QUESTION or MESSAGE, ALWAYS ONLY PROVIDE YOUR MOST ACCURATE ANSWER!
...
def prompt4Data(prompt, context, solution):
    final_prompt = f"""GENERAL INFORMATION : You is built by RapidAgent.ai
                        INSTRUCTION : IN YOUR ANSWER NEVER INCLUDE THE USER QUESTION or MESSAGE, YOU MUST MAKE THE CORRECT ANSWER MORE ARGUMENTED ! IF THE CORRECT ANSWER CONTAINS CODE YOU ARE OBLIGED TO INSERT IT IN YOUR NEW ANSWER!
                        PREVIUS MESSAGE : ({context})
                        NOW THE USER ASK : {prompt}
...
def prompt4Code(prompt, context, solution):
    final_prompt = f"""GENERAL INFORMATION : You are built by RapidAgent.ai
                        INSTRUCTION : IN YOUR ANSWER NEVER INCLUDE THE USER QUESTION or MESSAGE, THE CORRECT ANSWER CONTAINS CODE YOU ARE OBLIGED TO INSERT IT IN YOUR NEW ANSWER!
                        PREVIUS MESSAGE : ({context})
                        NOW THE USER ASK : {prompt}
